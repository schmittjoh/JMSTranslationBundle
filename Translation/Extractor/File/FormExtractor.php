...
class FormExtractor implements FileVisitorInterface, \PHPParser_NodeVisitor
{
    ...
    public function enterNode(\PHPParser_Node $node)
    {
        ...
        if ($node instanceof \PHPParser_Node_Expr_MethodCall) {
            ...
            foreach ($node->args[2]->value->items as $item) {
                ...
                if ('label' !== $item->key->value && 'empty_value' !== $item->key->value && 'choices' !== $item->key->value && 'attr' !== $item->key->value && 'invalid_message' !== $item->key->value) {
                    continue;
                }
                ...
                if (	'label' 			!== $item->key->value &&
                		'empty_value'		!== $item->key->value &&
                		'attr'				!== $item->key->value &&
                		'choices' 			!== $item->key->value &&
                		'first_options' 	!== $item->key->value &&
                		'second_options'	!== $item->key->value &&
    ...
    private function parseItem($item, $domain = null)
    {
        ...
        if (!$item->value instanceof \PHPParser_Node_Scalar_String) {
			...
			echo "trying to extract string from ";
			echo print_r($item->value, true)."\n";
